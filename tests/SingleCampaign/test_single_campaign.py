...
def test_set_distributor(bob, distributor, test_gauge, single_campaign):
    ...
    single_campaign.setup(
        distributor.address, test_gauge.address, min_epoch_duration, 1, "test", sender=bob
    )
...
def test_set_distributor_revert_not_manager(alice, distributor, test_gauge, single_campaign):
    ...
    with ape.reverts("only guards can call this function"):
        single_campaign.setup(
            distributor.address, test_gauge.address, min_epoch_duration, 1, "test", sender=alice
        )
...
def test_distribution_after_set_epochs(
    alice, bob, charlie, distributor, single_campaign, reward_token, test_gauge, chain
):
    epochs = [1 * 10**18, 2 * 10**18, 3 * 10**18]
    ...
    single_campaign.setup(
        distributor.address, test_gauge.address, min_epoch_duration, 1, "test", sender=charlie
    )
...
def test_set_distributor_revert_not_owner(alice, distributor, test_gauge, single_campaign):
    with ape.reverts("only guards can call this function"):
        single_campaign.setup(
            distributor.address, test_gauge.address, 3 * DAY, 1, "test", sender=alice
        )


def test_distribution_order(
    alice, bob, charlie, distributor, single_campaign, reward_token, test_gauge, chain
):
...
def test_distribution_timing(alice, bob, charlie, distributor, single_campaign, test_gauge, chain):
    ...
    chain.pending_timestamp = (
        chain.pending_timestamp + min_epoch_duration - DISTRIBUTION_BUFFER - 1000
    )
    chain.mine()
...
def test_distribute_revert_no_epochs(
    alice, bob, charlie, distributor, single_campaign, test_gauge, chain
):
    epochs = [2 * 10**18, 1 * 10**18, 5 * 10**18]
    min_epoch_duration = 3 * DAY
    single_campaign.set_reward_epochs(epochs, sender=charlie)
...
def test_get_next_epoch_info_revert_no_epochs(
    alice, bob, charlie, distributor, single_campaign, test_gauge, chain
):
    epochs = [2 * 10**18, 1 * 10**18, 5 * 10**18]
    single_campaign.set_reward_epochs(epochs, sender=charlie)
...
def test_remaining_epochs_count(
    alice, bob, charlie, distributor, single_campaign, test_gauge, chain
):
    epochs = [2 * 10**18, 1 * 10**18, 5 * 10**18]
    min_epoch_duration = 4 * DAY
...
def test_setup_with_custom_epoch_duration(bob, distributor, test_gauge, single_campaign):
    ...
    single_campaign.setup(
        distributor.address, test_gauge.address, custom_duration, 1, "test", sender=bob
    )
...
def test_setup_revert_epoch_too_short(bob, distributor, test_gauge, single_campaign):
    ...
    with ape.reverts("epoch duration must be between 3 days and a year"):
        single_campaign.setup(
            distributor.address, test_gauge.address, too_short, 1, "test", sender=bob
        )
...
def test_setup_revert_epoch_too_long(bob, distributor, test_gauge, single_campaign):
    ...
    with ape.reverts("epoch duration must be between 3 days and a year"):
        single_campaign.setup(
            distributor.address, test_gauge.address, too_long, 1, "test", sender=bob
        )


def test_distribution_respects_min_epoch_duration(
    bob, charlie, distributor, single_campaign, test_gauge, chain
):
    ...
    chain.pending_timestamp = (
        chain.pending_timestamp + custom_duration + DISTRIBUTION_BUFFER + 2000
...
def test_distribution_buffer(chain, bob, charlie, distributor, test_gauge, single_campaign):
    ...
    chain.pending_timestamp = (
        chain.pending_timestamp + WEEK - DISTRIBUTION_BUFFER - 1000
    )  # Just before buffer window
    chain.mine()
...
def test_execution_allowed_no_balance(
    bob, charlie, distributor, single_campaign, test_gauge, chain
):
...
def test_execute(
    bob, charlie, distributor, single_campaign, test_gauge, chain, crvusd_token, reward_token
):
    ...
    chain.pending_timestamp = (
        chain.pending_timestamp + min_epoch_duration - single_campaign.DISTRIBUTION_BUFFER() + 2
    )
    chain.mine()
...
def test_execute_without_crvusd(
    bob, charlie, distributor, single_campaign, test_gauge, chain, crvusd_token
):
...
def test_next_execution_times(bob, charlie, distributor, single_campaign, test_gauge, chain):
    ...
    assert (
        single_campaign.next_execution_allowed_time_buffer()
        == min_epoch_duration - DISTRIBUTION_BUFFER
    )
...
def test_next_execution_payment_amount(
    bob, charlie, distributor, single_campaign, test_gauge, crvusd_token
):
...
def test_end_campaign_revert_not_guard(
    alice, bob, charlie, distributor, single_campaign, test_gauge
):
...
def test_remove_active_campaign_address_method(
    bob, charlie, distributor, single_campaign, test_gauge
):
...
def test_remove_active_campaign_address_revert_not_guard(
    alice, bob, charlie, distributor, single_campaign, test_gauge
):
...
def test_remove_active_campaign_address_revert_not_guard_last(
    alice, bob, charlie, distributor, single_campaign, test_gauge
):
...
def test_remove_active_campaign_address_revert_not_set(
    bob, distributor, single_campaign, test_gauge
):
