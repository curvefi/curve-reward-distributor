...
def deposit_reward_token(
    ...
    assert extcall IERC20(self.reward_token).transferFrom(
        msg.sender, self, _amount, default_return_value=True
    )
...
def recover_token() -> bool:
    ...
    if amount > 0:
        assert extcall IERC20(self.reward_token).transfer(
            self.recovery_address, amount
        )
        return True
    else:
        return False
